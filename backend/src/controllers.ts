...
export async function findPropertiesByCoordinates(req: Request<{}, {}, GeoJSONPoint>, res: Response) {
  try {
    ...
    const result = await pool.query(query, [longitude, latitude, searchDistance]);
...
export async function displayPropertyImage(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const { overlay, parcel, building } = req.query;
    
    // Valid CSS color names (includes examples from spec: red, green, orange)
    // Normalize colors if provided (validation handled by middleware)
    const normalizedParcelColor = parcel !== undefined && typeof parcel === 'string' ? parcel.toLowerCase() : undefined;
    const normalizedBuildingColor = building !== undefined && typeof building === 'string' ? building.toLowerCase() : undefined;
    ...
    if (overlay === 'yes' && (normalizedParcelColor || normalizedBuildingColor)) {
...
function createPolygonSVG(geoJson: any, imageBounds: number[], width: number, height: number, color: string): string {
